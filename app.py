...
# 2. 테마 및 UI 스타일 세팅
# ==========================================
st.set_page_config(page_title="APEX V40.0 - Anti FOMO", layout="wide", page_icon="⚖️")
...
def get_ranking_data(tickers, k, allocated_budget, gap_limit, sl_pct, base_rr):
    ...
    for ticker in tickers:
        try:
            ...
            # [핵심] 추격매수 금지 로직 (타점 대비 1.5% 초과 상승 시)
            ...
            elif is_chasing: score -= 30; reasons.append("⛔추격금지") # 추격매수 강등 처리
            else:
...
with tab1:
    if selected_sector:
        ...
        else: st.info("현재 상승장 및 진입 가능한 타점(추격매수 제외)을 완벽히 충족하는 종목이 없습니다.")
...
# 안전한 스무스 싱크용 히든 버튼 (새로고침 대신 데이터만 갱신)
