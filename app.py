...
TICKER_INFO = {
    ...
    "MCD": ("맥도날드", "소비재"), "NKE": ("나이키", "소비재"), "SBUX": ("스타벅스", "소비재"), "MELI": ("메르카도리브레", "이커머스"),
    "SE": ("씨리미티드", "이커머스"), "EBAY": ("이베이", "이커머스"), "ETSY": ("엣시", "이커머스"), "WAY": ("웨이페어", "유통"),
    "CHWY": ("츄이", "반려동물"), "PINS": ("핀터레스트", "SNS"), "FTCH": ("파페치", "명품"), "W": ("웨이페어", "가구"),
    # ... (생략된 종목들도 Ticker 기반으로 자동 처리되게 로직 추가)
}
...
def save_db(full_db):
    ...
    if key and bin_id and current_u:
        try:
            req_put = requests.put(f"https://api.jsonbin.io/v3/b/{bin_id}", data=json.dumps(full_db, cls=TacticalEncoder), headers={"Content-Type": "application/json", "X-Master-Key": key}, timeout=5)
        except: pass

# ==========================================
# 2. UI 및 압축 레이아웃 CSS
# ==========================================
st.set_page_config(page_title="APEX V63.0", layout="wide")
...
st.markdown("""
    ...
    /* 전역 타점 섹션 압축 */
    .global-hit-banner { background: linear-gradient(90deg, rgba(16,185,129,0.1) 0%, rgba(16,185,129,0) 100%); border-left: 4px solid #10b981; padding: 8px 15px; margin: 10px 0; font-weight: 800; color: #6ee7b7; }
    </style>
""", unsafe_allow_html=True)

# ==========================================
# 3. 데이터 매핑 함수 (Ticker -> Korean/Sector)
# ==========================================
def get_info(ticker):
...
if 'current_user' not in st.session_state:
    ...
    if st.button("START"):
        st.session_state.current_user = uid; st.rerun()
    st.stop()
...
with st.spinner("⚡ 전체 섹터 정찰 중..."):
    df_global = turbo_scan_engine(all_ticks, float(u_data['settings']['fixed_k']), float(u_data['settings']['stop_loss_pct']))
...
if not hits.empty:
    ...
    sel_hits = st.dataframe(hits[['Ticker', 'Name', 'Sector', 'Price', 'Chg']], on_select="rerun", selection_mode="single-row",
                            column_config={
                                ...
                                "Price": st.column_config.NumberColumn("가", format="$%.2f"),
...
with st.sidebar:
    ...
    if st.button("SAVE"):
        save_db(st.session_state.full_db); st.success("Saved.")
